...
import { addItem, incrementCartCounter, updateAddedToCart } from "./CartSlice";
function ProductList({ onHomeClick }) {
    ...
    //     }
    // });
    // cartCounter = cartCounter.reduce((total, count) => {
    //     (total + count, 0);
    // });
    ...
    const handleAddToCart = (selectedPlant) => {
        dispatch(addItem(selectedPlant));
        // setAddedToCart((prevState) => ({
        //     // Update the local state to reflect that the product has been added
        //     ...prevState, // Spread the previous state to retain existing entries
        //     [selectedPlant.name]: true, // Set the current product's name as a key with value 'true' to mark it as added
        // }));
        dispatch(
            updateAddedToCart({
        ...
        <div>
            ...
            {!showCart ? (
                <div className="product-grid">
                    {plantsArray.map((plantCategories, index) => (
                        <div key={index}>
                            <h2 className="plant_heading">
                                {plantCategories.category}
                            </h2>
                            <div className="product-list">
                                {plantCategories.plants.map((plant, index) => (
